...
research_agent = Agent("google-gla:gemini-1.5-flash",
...
@research_agent.tool
def search_in(search_data: RunContext[SearchDataclass], query: str, query_number: int) -> dict[str, Any]:
    """
    Get the search for keyword query.

    Args:
        query: keyword to search
    """
    max_results = search_data.deps.max_results
    results = DDGS().text(query=query, max_results=2 )
